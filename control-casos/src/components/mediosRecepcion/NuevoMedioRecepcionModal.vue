  ...
  <div class="modal-background" v-if="show" @click.self="closeModal">
    <div class="modal-content">
      <div class="modal-header">
        <h2>{{ modoEdicion ? 'Editar Medio de Recepción' : 'Nuevo Medio de Recepción' }}</h2>
...
export default {
  ...
  watch: {
    show(newVal) {
      if (newVal === true) {
        if (this.modoEdicion && this.medioSeleccionado) {
          // Si estamos en modo edición, cargamos los datos del medio seleccionado
          this.formData = {
            id: this.medioSeleccionado.id,
            descripcion: this.medioSeleccionado.descripcion
          };
        } else {
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      setTimeout(() => {
        if (this.modoEdicion) {
          this.$emit('update', datosMedioRecepcion);
        } else {
          this.$emit('submit', datosMedioRecepcion);
        }
      }, 50);
    },
    
    generarNuevoId() {
      // Si no hay medios de recepción, el nuevo ID será 1
      if (!this.mediosRecepcion || this.mediosRecepcion.length === 0) {
        this.formData.id = 1;
        return;
      }
      
      // Encuentra el ID más alto y añade 1
      const maxId = Math.max(...this.mediosRecepcion.map(medio => medio.id));
