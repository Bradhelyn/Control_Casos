  ...
  <div class="modal-background" v-if="show" @click.self="closeModal">
    <div class="modal-content">
      <div class="modal-header">
        <h2>{{ modoEdicion ? 'Editar Estado de Recomendación' : 'Nuevo Estado de Recomendación' }}</h2>
...
<script>
export default {
  name: 'NuevoEstadoRecomendacionModal',
  props: {
    show: {
      type: Boolean,
      default: false
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      const datosEstadoRecomendacion = { 
      ...
      setTimeout(() => {
        if (this.modoEdicion) {
          this.$emit('update', datosEstadoRecomendacion);
        } else {
          this.$emit('submit', datosEstadoRecomendacion);
        }
      }, 50);
    },
    
    generarNuevoId() {
      // Si no hay estados de recomendaciones, el nuevo ID será 1
      if (!this.estadosRecomendaciones || this.estadosRecomendaciones.length === 0) {
        this.formData.id = 1;
        return;
      }
      
      // Encuentra el ID más alto y añade 1
      const maxId = Math.max(...this.estadosRecomendaciones.map(estado => estado.id));
